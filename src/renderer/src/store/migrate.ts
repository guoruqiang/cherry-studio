...
const migrateConfig = {
  ...
  '172': (state: RootState) => {
    try {
      ...
      // Migrate sidebar icons - keep 'agents' as is (no migration to 'store')
      // Note: 'store' is not a valid SidebarIcon type, so we keep 'agents'
      if (state.settings.sidebarIcons) {
        // Filter out invalid icons like 'store' if any exist
        const validIcons: SidebarIcon[] = ['assistants', 'agents', 'translate', 'knowledge', 'files', 'notes', 'code_tools']
        state.settings.sidebarIcons.visible = state.settings.sidebarIcons.visible.filter((icon): icon is SidebarIcon => {
          const iconStr = icon as string
          return iconStr !== 'store' && validIcons.includes(icon as SidebarIcon)
        })
        state.settings.sidebarIcons.disabled = state.settings.sidebarIcons.disabled.filter((icon): icon is SidebarIcon => {
          const iconStr = icon as string
          return iconStr !== 'store' && validIcons.includes(icon as SidebarIcon)
        })
      }
